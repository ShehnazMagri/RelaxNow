  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  getCorporateAssesment(): void {
    const params = {
      query: `Call RN_GetAllCorporateConfigurationAssesment()`,
      params: '',
    };
    this.http.postData(ApiUrl.common, params).subscribe(
      (resp: any) => {
  ...
  openQuestionModal(template: TemplateRef<any>, item): void {
    ...
    this.selectedCorporateCode = item.AssesmentCode;
  }
  ...
  onTestSelect(item) {
    var data = this.corporateListData.filter(
      (x) => x.companyID == this.companyId
    );
    this.code = item.code;
  }

  openViewReportModal(template: TemplateRef<any>, item): void {
    this.modalRef = this.modalService.show(template, {
      class: 'modal-sm2 modal-lg modal-xl',
    });

    this.reportAssesmentID = item.id;
    this.reportCorporateID = item.AssesmentCode;
  }
}
